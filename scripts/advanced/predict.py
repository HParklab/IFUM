...
#
# =====================================================================================
# MERGED SCRIPT: predict.py (Updated for model_20250626)
#
# This script combines the functionality of preparation and execution.
#
...
# 3. Execution:
#    - Immediately runs IEFFEUM on the prepared files.
#    - Outputs a final CSV file with stability (dG), stability class, and SAP predictions.
#
# Original sources:
...
# The utils script should contain helper functions for the new model
from IEFFEUM.advanced import utils
...
def create_arg_parser():
    ...
    parser.add_argument(
        ...
        help='Report per-residue dG and SAP contributions in the output CSV and input PDBs/CIFs. (default: False)'
    )
    parser.add_argument(
...
def prepare_IEFFEUM_inputs(seq_embd, pdb_embd, pt_dir):
    ...
    for name in tqdm(seq_embd.keys(), desc="Creating .pt files"):
        if name not in pdb_embd:
            ...
            logger.warning(f"PDB/CIF names: {list(pdb_embd.keys())}")
            logger.warning(f"FASTA names: {list(seq_embd.keys())}")
            continue
...
def write_per_resi_to_pdb(pdb_path, names, p_dGs_per_resi, p_saps_per_resi):
    """Appends per-residue dG and SAP values as comments to PDB/CIF files."""
    structure_files = []
    ...
    for struct_file, dgs, saps in zip(structure_files, p_dGs_per_resi, p_saps_per_resi):
        with open(struct_file, 'a') as f: # Open in append mode
            f.write('#\n# Per-residue dG predictions:\n')
            for i, dg in enumerate(dgs):
                f.write(f'# per_resi_dg_{i+1} {float(dg):.3f}\n')
            f.write('#\n# Per-residue SAP predictions:\n')
            for i, sap in enumerate(saps):
                f.write(f'# per_resi_sap_{i+1} {float(sap):.3f}\n')
...
if __name__ == '__main__':
    ...
    if input_path.is_file() and input_path.suffix.lower() == '.fasta':
        ...
        seq_dict = {fasta_entry[0]: fasta_entry[1] for fasta_entry in read_fasta(input_path)}
    ...
    model_input_dict = prepare_IEFFEUM_inputs(seq_embd, pdb_embd, pt_dir)
    if not model_input_dict['name']:
        logger.error("No valid inputs could be prepared for IEFFEUM. Exiting.")
        sys.exit(1)

    # Create the list file in memory for the next step
    list_file_path = temp_dir / 'model_inputs.list'
    torch.save(model_input_dict, list_file_path)
    ...
    with torch.no_grad():
        dataloader, model = utils.get_dataloader_and_model(list_file_path, args.model_path, device, int(args.batch_size))

        NAMES, P_DGS, P_DGS_PER_RESI, P_SAP_PER_RESI = [], [], [], []
        ...
        for batch in tqdm(dataloader, desc="Running IEFFEUM Inference"):
            ...
            _, (p_dgs, p_dgs_per_resi), _, p_sap_per_resi, _ = model(target_Fs_onehot, seq_embds, str_embds, mask_1ds, mask_2ds)
            NAMES, P_DGS, P_DGS_PER_RESI, P_SAP_PER_RESI = utils.gather_batch_results(names, p_dgs, p_dgs_per_resi, p_sap_per_resi, NAMES, P_DGS, P_DGS_PER_RESI, P_SAP_PER_RESI)
    ...
    if args.per_resi:
        # --- Updated PDB/CIF Writing Call ---
        write_per_resi_to_pdb(pdb_path_for_emb, NAMES, P_DGS_PER_RESI, P_SAP_PER_RESI)
        logger.info("Writing per-residue dG and SAP contributions to PDB/CIF files.")
