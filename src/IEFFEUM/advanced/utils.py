...
from IEFFEUM.advanced import dataset, model
...
def gather_batch_results(names, p_dGs, p_dGs_per_resi, p_sap_per_resi, NAMES, P_DGS, P_DGS_PER_RESI, P_SAP_PER_RESI):
    NAMES.extend(names)
    P_DGS.extend(p_dGs.detach().cpu().numpy().round(3))
    P_DGS_PER_RESI.extend(p_dGs_per_resi[0].detach().cpu().numpy().round(3))
    P_SAP_PER_RESI.extend(p_sap_per_resi[0].detach().cpu().numpy().round(3))
    
    return NAMES, P_DGS, P_DGS_PER_RESI, P_SAP_PER_RESI
