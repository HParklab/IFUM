...
class DistogramHead(nn.Module):
    def __init__(self, dim,):
        super().__init__()
        self.to_distogram = FeedForward(dim)
        
    def forward(self, x, mask):
        x = self.to_distogram(x) + x
        x = x * rearrange(mask, 'b i j -> b i j ()')
        
        return symmetrize_tensor(x)

class FoldingStabilityHeadNLL(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.to_stability_mean = FeedForward(dim)
        self.to_stability_var = FeedForward(dim)
        
    def forward(self, s, seq_mask):
        x_mean = self.to_stability_mean(s) + s
        x_v = self.to_stability_var(s) + s
        
        x_mean = x_mean * rearrange(seq_mask, 'b n i -> b n i ()', n=2)
        x_v = torch.exp(x_v) * rearrange(seq_mask, 'b n i -> b n i ()', n=2)
        
        per_res_stability_mean = reduce(x_mean, 'b n i d -> b 1 i', 'sum', n=2)
        p_v = reduce(x_v, 'b n i d -> b 1 i', 'sum', n=2)
        
        folding_stability_mean = reduce(per_res_stability_mean, 'b 1 i -> b 1', 'sum')
        v = reduce(p_v, 'b 1 i -> b 1', 'sum')
        
        return (folding_stability_mean, v), (per_res_stability_mean, p_v)

class SequenceHead(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.to_sequence = nn.ModuleList([
            FeedForward(dim),
            nn.Linear(dim, 20),
            ])
        
    def forward(self, x, seq_mask):
        ff, out =  self.to_sequence
...
class IEFFEUM(nn.Module):
    def __init__(
        self, depth=11, dim=21, num_heads=4, dim_head=8, attn_dropout=0, ff_dropout=0):
        super().__init__()
        self.reshape_f_seq = DiffAttn3d(in_dim=1024, out_dim=dim,)
        self.reshape_f_bb = DiffAttn3d(in_dim=512, out_dim=dim,)
        ...
        self.folding_stability_head = FoldingStabilityHeadNLL(dim)
        self.sequence_head = SequenceHead(dim)
        
    def forward(self, x:Tensor, s_1d:Tensor, s_2d:Tensor, mask_1d:Tensor, mask_2d:Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        s_1d = self.reshape_f_seq(s_1d, mask_2d)
        s_2d = self.reshape_f_bb(s_2d, mask_2d)
        s = torch.cat([s_1d, s_2d], dim=1)
        ...
        return distogram, folding_stability, per_res_stability, seq
